...
func place_painting_near_wall(pos_x, pos_z, is_vertical):
	...
	painting_instance.scale = Vector3(0.4, 0.4, 0.4)
	add_child(painting_instance)
	
func place_dresser_under_painting(pos_x, pos_z, is_vertical):
	var dresser_scene = preload("res://scenes/Dresser.tscn")
	var dresser_instance = dresser_scene.instantiate()
	var offset = Vector3(0, -0.35, 0)  # Adjust as needed
	if is_vertical:
		offset = Vector3(0.06, 0, 0)  # Example offset for vertical wall
		dresser_instance.rotate_x(deg_to_rad(90))
		dresser_instance.rotate_y(deg_to_rad(90))
	else:
		offset = Vector3(0, 0.1, -0.12)
		#dresser_instance.rotate_x(deg_to_rad(90))
		dresser_instance.rotate_y(deg_to_rad(180))
	dresser_instance.transform.origin = Vector3(pos_x, 0, pos_z) + offset
	dresser_instance.scale = Vector3(0.15, 0.15, 0.15)
	if randf() < 0.4:
		var candle_scene = preload("res://scenes/Candle.tscn")
		var candle_instance = candle_scene.instantiate()
		candle_instance.scale = Vector3(0.1, 0.1, 0.1)
		#candle_instance.transform.origin = dresser_instance.transform.origin + Vector3(randf_range(-0.075, 0.075), 0.170, randf_range(-0.025, 0.025))
		#candle_instance.transform.origin = Vector3(pos_x, 0, pos_z) + offset
		candle_instance.transform.origin = Vector3(randf_range(-0.5, 0.5), 1.1, randf_range(-0.2, 0.2))
		dresser_instance.add_child(candle_instance)
	dresser_instance.connect("game_win", $"../Player".on_game_win)
	add_child(dresser_instance)
...
func place_horizontal_wall(cell_index_1, cell_index_2):
	...
	if randf() < 0.4:
		place_dresser_under_painting(pos_x, pos_z, false)
...
func place_floor_tiles():
	var floor_scene = preload("res://scenes/floor_instance.tscn")
